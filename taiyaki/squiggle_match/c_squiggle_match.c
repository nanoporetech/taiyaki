...
float squiggle_match_forward(float const * signal, size_t nsample, float const * params, size_t ldp,
                             float const * scale, size_t npos, float prob_back, float * fwd){
    assert(nsample > 0);
    assert(npos > 0);
    assert(NULL != signal);
    assert(NULL != params);
    ...
    for(size_t sample=0 ; sample < nsample ; sample++){
        ...
        for(size_t pos=0 ; pos < npos ; pos++){
            ...
            fwd[fwd_curr_off + npos + pos] = fwd[fwd_prev_off + npos + pos] + stay_in_back_pen;
        }
        for(size_t pos=1 ; pos < npos ; pos++){
            //  Move to next position
            fwd[fwd_curr_off + pos] = logsumexp(fwd[fwd_curr_off + pos],
                                                fwd[fwd_prev_off + pos - 1] + move_pen[pos]);
        }
        for(size_t pos=1 ; pos < npos ; pos++){
            // Move backwards
            fwd[fwd_curr_off + npos + pos - 1] = logsumexp(fwd[fwd_curr_off + npos + pos - 1],
                                                           fwd[fwd_prev_off + pos] + move_back_pen);
        }
        for(size_t pos=1 ; pos< npos ; pos++){
            // Move from back state
            fwd[fwd_curr_off + pos] = logsumexp(fwd[fwd_curr_off + pos],
                                                fwd[fwd_prev_off + npos + pos - 1] + move_from_back_pen);
        }
        ...
        for(size_t pos=0 ; pos < npos ; pos++){
            ...
            const float logscore = loglaplace(signal[sample], location, scale[pos], logscale);
...
float squiggle_match_backward(float const * signal, size_t nsample, float const * params, size_t ldp,
                              float const * scale, size_t npos, float prob_back, float * bwd){
    assert(nsample > 0);
    assert(npos > 0);
    assert(NULL != signal);
    assert(NULL != params);
    ...
    for(size_t sample=nsample ; sample > 0 ; sample--){
        ...
        for(size_t pos=0 ; pos < npos ; pos++){
            const float location = params[pos * ldp + 0];
            const float logscale = params[pos * ldp + 1];
            const float logscore = loglaplace(signal[sample - 1], location, scale[pos], logscale);
...
float squiggle_match_viterbi(float const * signal, size_t nsample, float const * params, size_t ldp,
                             float const * scale, size_t npos, float prob_back, float localpen,
                             float minscore, int32_t * path, float * fwd){
    assert(nsample > 0);
    assert(npos > 0);
    assert(NULL != signal);
    assert(NULL != params);
    ...
    for(size_t sample=0 ; sample < nsample ; sample++){
        ...
        for(size_t pos=0 ; pos < npos ; pos++){
            ...
            const float location = params[pos * ldp + 0];
            const float logscale = params[pos * ldp + 1];
            const float logscore = fmaxf(-minscore, loglaplace(signal[sample], location, scale[pos], logscale));
    ...
    const float score = fmaxf(fwd[fwd_offset + nfstate - 2], fwd[fwd_offset + nfstate - 1]);
...
#pragma omp parallel for
    for(size_t batch=0 ; batch < nbatch ; batch++){
        ...
        score[batch] = squiggle_match_forward(signal + signal_offset, nsample, params + param_offset,
                                              ldp, scale, npos, prob_back, fwd);
        free(scale);
        free(fwd);
    }
}
...
void squiggle_match_scores_fwd(float const * signal, int32_t const * siglen, size_t nbatch,
                               float const * params, size_t npos, float prob_back, float * score){
    squiggle_match_cost(signal, siglen, nbatch, params, npos, prob_back, score);
}


void squiggle_match_scores_bwd(float const * signal, int32_t const * siglen, size_t nbatch,
                               float const * params, size_t npos, float prob_back, float * score){
    size_t sigidx[nbatch];
    sigidx[0] = 0;
    for(size_t idx=1 ; idx < nbatch ; idx++){
...
#pragma omp parallel for
    for(size_t batch=0 ; batch < nbatch ; batch++){
        ...
        score[batch] = squiggle_match_backward(signal + signal_offset, nsample, params + param_offset,
                                               ldp, scale, npos, prob_back, bwd);
        free(scale);
        free(bwd);
    }
}
...
#pragma omp parallel for
    for(size_t batch=0 ; batch < nbatch ; batch++){
        ...
        score[batch] = squiggle_match_viterbi(signal + signal_offset, nsample, params + param_offset,
                                              ldp, scale, npos, prob_back, localpen, minscore,
...
float squiggle_match_posterior(float const * signal, size_t nsample, float const * params, size_t ldp,
                               float const * scale, size_t npos, float prob_back, float * post){
    const size_t nstate = 2 * npos;
    float * fwd = post;
    float * bwd = calloc(nstate * (nsample + 1), sizeof(float));
    float score = squiggle_match_forward(signal, nsample, params, ldp, scale, npos, prob_back, fwd);
    squiggle_match_backward(signal, nsample, params, ldp, scale, npos, prob_back, bwd);
...
#pragma omp parallel for
    for(size_t batch=0 ; batch < nbatch ; batch++){
        ...
        squiggle_match_forward(signal + signal_offset, nsample, params + param_offset,
                               ldp, scale, npos, prob_back, fwd);
        squiggle_match_backward(signal + signal_offset, nsample, params + param_offset,
                                ldp, scale, npos, prob_back, bwd);
        ...
        for(size_t sample=1 ; sample <= nsample ; sample++){
            ...
            for(size_t pos=0 ; pos < npos ; pos++){
                ...
                const float prob_pos = expf(fwd[offset + pos] + bwd[offset + pos] - fact);
                const float prob_posnpos = expf(fwd[offset + npos + pos] + bwd[offset + npos + pos] - fact);
                grad[param_offset + pos * ldp + 0] += (prob_pos + prob_posnpos) * dloglaplace_loc(sig, loc, scale[pos], logsc);
                grad[param_offset + pos * ldp + 1] += (prob_pos + prob_posnpos) * dloglaplace_logscale(sig, loc, scale[pos], logsc);
            }
            ...
            for(size_t pos=0 ; pos < npos ; pos++){
                ...
                const float pprob_pos = expf(fwd[offset - nstate + pos] + bwd[offset + pos] + logem - fact);
                const float move_pen = plogisticf(params[param_offset + pos * ldp + 2]);
                const float dlogisticf_move_pen = (1.0f - prob_back) * move_pen * (1.0f - move_pen);
                grad[param_offset + pos * ldp + 2] -= pprob_pos * dlogisticf_move_pen;
            }
            ...
            for(size_t pos=1 ; pos < npos ; pos++){
                ...
                const float logsc = params[param_offset + pos * ldp + 1];
                const float logem = loglaplace(sig, loc, scale[pos], logsc);
                const float pprob_pos = expf(fwd[offset - nstate + pos - 1] + bwd[offset + pos] + logem - fact);
                const float move_pen = plogisticf(params[param_offset + pos * ldp + 2]);
                const float dlogisticf_move_pen = (1.0f - prob_back) * move_pen * (1.0f - move_pen);
                grad[param_offset + pos * ldp + 2] += pprob_pos * dlogisticf_move_pen;
            }
...
int main(void){
    ...
    squiggle_match_scores_fwd(test_signal, test_siglen, nbatch, test_param, npos, prob_back, score);
    printf("Forwards scores: %f %f\n", score[0], score[1]);

    squiggle_match_scores_bwd(test_signal, test_siglen, nbatch, test_param, npos, prob_back, score);
    printf("Backwards scores: %f %f\n", score[0], score[1]);

    squiggle_match_viterbi_path(test_signal, test_siglen, nbatch, test_param, npos, prob_back, localpen,
                                minscore, path, score);
    ...
    squiggle_match_grad(test_signal, test_siglen, nbatch, test_param, npos, prob_back, grad);
    float maxdelta = 0.0;
    for(size_t pos=0 ; pos < npos ; pos++){
        const size_t offset = pos * nbatch *nparam;
        for(size_t st=0 ; st < nparam ; st++){
            maxdelta = fmaxf(maxdelta, fabsf(grad[offset + st] - grad[offset + nparam + st]));
        }
    }
    ...
    for(size_t pos=0 ; pos < npos  ; pos++){
        ...
        for(size_t st=0 ; st < nparam ; st++){
            ...
            squiggle_match_scores_fwd(test_signal, test_siglen, nbatch, test_param, npos, prob_back, score);
            printf("    %f d=%f [%f %f] (%f %f)\n", grad[offset + st], fabsf(grad[offset + st] - fscore[0]), fscore[0], fscore[1], score[0], score[1]);
    ...
    for(size_t pos=0 ; pos < npos ; pos++){
        ...
        for(size_t sample=0 ; sample < test_siglen[0] ; sample++){
            ...
            const float df = dloglaplace_logscale(test_signal[sample], loc, expf(logsc), logsc);
            const float dplus = loglaplace(test_signal[sample], loc, expf(logsc + DELTA), logsc + DELTA);
            const float dminus = loglaplace(test_signal[sample], loc, expf(logsc - DELTA), logsc - DELTA);
            const float approxdf = (dplus - dminus) / (2.0f * DELTA);
            printf("dlog/dloc = %f\t%f\t%f\n", df, approxdf, fabsf(df - approxdf));
        }
    }
}
#endif
