...
float crf_flipflop_forward_step(float const *restrict logprob,
                                ...
                                int32_t const *restrict moveidx,
                                int32_t const *restrict stayidx,
    ...
    for (size_t pos = 1; pos < nseqpos; pos++) {
        const float score = fwdprev[pos - 1] + logprob[moveidx[pos - 1]];
        fwdcurr[pos] = logsumexpf(fwdcurr[pos], score, sharpfact);
        // in non-log space, we now have
        // fwdcurr[s] = fwdprev[s] * stayprob + fwdprev[s-1] * stepprob
    }

    const float factor = logsumexpf_vec(fwdcurr, nseqpos);
    for(size_t pos=0 ; pos < nseqpos ; pos++){
        fwdcurr[pos] -= factor;
    }
    return factor;
...
float crf_flipflop_forward(float const *restrict logprob, size_t nblk,
                           size_t ldp, int32_t const *restrict moveidx,
                           int32_t const *restrict stayidx, size_t nseqpos,
    ...
    for (size_t blk = 0; blk < nblk; blk++) {
        ...
        score += crf_flipflop_forward_step(logprobcurr, fwdprev, moveidx, stayidx,
                                           nseqpos, fwdcurr, sharpfact);
    }

    // Final score is sum of final state + its stay
    score += fwd[nblk * nseqpos + nseqpos - 1];
    return score;
}
...
float crf_flipflop_backward_step(float const *restrict logprob,
    ...
    for (size_t pos = 1; pos < nseqpos; pos++) {
        ...
        const float score = bwdprev[pos] + logprob[moveidx[pos - 1]];
        bwdcurr[pos - 1] = logsumexpf(bwdcurr[pos - 1], score, sharpfact);
    }

    const float factor = logsumexpf_vec(bwdcurr, nseqpos);
    for(size_t pos=0 ; pos < nseqpos ; pos++){
        bwdcurr[pos] -= factor;
    }
    return factor;
...
float crf_flipflop_backward(float const *restrict logprob, size_t nblk,
    ...
    for (size_t blk = nblk; blk > 0; blk--) {
        ...
        score += crf_flipflop_backward_step(logprobcurr, bwdprev, moveidx, stayidx,
                                            nseqpos, bwdcurr, sharpfact);
    }
...
void crf_flipflop_grad_step(float const *restrict fwdcurr,
                            ...
                            int32_t const *restrict moveidx,
                            int32_t const *restrict stayidx, size_t nseqpos,
                            float *restrict grad, size_t ntrans, float fact,
                            float sharpfact) {
    ...
    float gradtmp[nseqpos + nseqpos - 1];
    for (size_t pos = 0; pos < nseqpos; pos++) {
...
#pragma omp parallel for
    ...
    for (size_t batch = 0; batch < nbatch; batch++) {
        ...
        int32_t const *moveidx = moveidxs + seqidx[batch] - batch;
        int32_t const *stayidx = stayidxs + seqidx[batch];
        ...
        for (size_t blk = 0; blk < nblk; blk++) {
            ...
            crf_flipflop_grad_step(fwdcurr, bwdnext, logprobcurr, moveidx, stayidx,
                                   nseqpos, gradcurr, ntrans, score[batch], sharpfact);
        }
...
const int32_t test_move1[12] = { 8, 33, 13, 25, 19,
    8, 33, 13, 25, 19
};

const int32_t test_stay1[12] = { 0, 9, 37, 9, 27, 18,
    0, 9, 37, 9, 27, 18
};
