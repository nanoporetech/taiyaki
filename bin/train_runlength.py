...
def main():
    ...
    for i in range(args.niteration):
        ...
        # Sequence input tensor is just a 1D vector, and so is seqlens and
        # seqrles
        seqs_rle = [rle(d['sequence']) for d in chunk_batch]
        seqlens = torch.tensor([len(elt[0]) for elt in seqs_rle],
                               dtype=torch.long, device=device)
        seqrles = torch.tensor(np.concatenate([elt[1] for elt in seqs_rle]),
                               dtype=torch.long, device=device)
        seqs = torch.tensor(np.concatenate([elt[0] for elt in seqs_rle]),
                            dtype=torch.long, device=device)
        ...
        total_bases += sum([len(d['sequence']) for d in chunk_batch])
        total_samples += int(indata.nelement())

        # Doing this deletion leads to less CUDA memory usage.
        del indata, seqs, seqlens, seqrles, outputs, loss, lossvector
        if device.type == 'cuda':
