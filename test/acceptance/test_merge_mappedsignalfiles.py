...
class AcceptanceTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.merge_script = os.path.join(util.MISC_DIR, "merge_mappedsignalfiles.py")
        self.plot_script = os.path.join(util.MISC_DIR, "plot_mapped_signals.py")
        self.mapped_signal_file0 = os.path.join(util.DATA_DIR, "mapped_signal_file/mapped_reads_0.hdf5")
        self.mapped_signal_file1 = os.path.join(util.DATA_DIR, "mapped_signal_file/mapped_reads_1.hdf5")
    ...
    def count_reads(self, mapped_signal_file, print_readlist = True):
        """Count the number of reads in a mapped signal file, using the script
        plot_mapped_signals.py"""
        cmd = [self.plot_script,
               'none', #don't save a png
               mapped_signal_file]
        readlist = util.run_cmd(self, cmd).get_stdout()
        if print_readlist:
            print("Read list:")
            print(readlist)
        #Count number of occurrences of the word 'read' in the output
        numreads = (''.join(readlist)).count('read')
        return numreads


    def test_merge(self):
        test_work_dir = self.work_dir("test_merge")
        merged_mapped_signal_file = os.path.join(test_work_dir, 'merged_mappedsignalfile.hdf5')
        print("Looking for ",self.mapped_signal_file0, '(abs)=', os.path.abspath(self.mapped_signal_file0))
        print("Result to be placed in",merged_mapped_signal_file, '(abs)=', os.path.abspath(merged_mapped_signal_file))
        ...
        print("Total number of reads in files to be merged = {} + {} =  {}".format(
